  ...
  public ChannelReader validateArticles(String exerciseId, String channelId, User user) {
    ...
    Map<String, Instant> toPublishArticleIdsMap =
        injects.stream()
            ...
            .sorted(
                Comparator.comparing(inject -> inject.getExecution().get().getTrackingSentDate())) //TODO POC
            .flatMap(
                inject -> {
                  Instant virtualInjectDate = inject.getExecution().get().getTrackingSentDate(); //TODO POC
                  try {
