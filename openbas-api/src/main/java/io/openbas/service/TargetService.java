...
public class TargetService {
    ...
    public Page<InjectTarget> injectTargets(TargetType injectTargetType, Inject inject, SearchPaginationInput input) {
        Page<InjectTarget> tgts;
        switch (injectTargetType) {
            case ASSETS_GROUPS:
                Filters.FilterGroup newFilterGroup = convertTargetFiltersToAssetGroupFilters(input.getFilterGroup(), inject);
                input.setFilterGroup(newFilterGroup);
                Page<AssetGroupOutput> filteredAssetGroups = assetGroupCriteriaBuilderService.assetGroupPagination(input);
                Page<InjectTarget> filteredTargets = new PageImpl<>(filteredAssetGroups.getContent().stream().map(this::convertFromAssetGroupOutput).toList(), filteredAssetGroups.getPageable(), filteredAssetGroups.getTotalElements());
    ...
    private InjectTarget convertFromAssetGroupOutput(AssetGroupOutput assetGroupOutput) {
        return new AssetGroupTarget(assetGroupOutput.getId(), assetGroupOutput.getName(), assetGroupOutput.getTags());
    }

    private Filters.FilterGroup convertTargetFiltersToAssetGroupFilters(Filters.FilterGroup filterGroup, Inject inject) {
