...
public class CalderaInjector {
    ...
    public CalderaInjector(InjectorService injectorService, CalderaContract contract, CalderaInjectorConfig calderaInjectorConfig, OpenBASConfig openBASConfig) {
        ...
        executorCommands.put(Endpoint.PLATFORM_TYPE.Linux.name() + "." + Endpoint.PLATFORM_ARCH.x86_64, "x=\"#{location}\";location=$(echo \"$x\" | sed \"s#/openbas-caldera-agent##\");filename=obas-implant-caldera-$(tr -dc A-Za-z0-9 </dev/urandom | head -c 5; echo);server=\"" + calderaInjectorConfig.getPublicUrl() + "\";curl -s -X GET " + openBASConfig.getBaseUrl() + "/api/implant/caldera/linux/x86_64 > $location/$filename;chmod +x $location/$filename;$location/$filename -server $server -group red &");
        executorCommands.put(Endpoint.PLATFORM_TYPE.MacOS.name() + "." + Endpoint.PLATFORM_ARCH.x86_64, "x=\"#{location}\";location=$(echo \"$x\" | sed \"s#/openbas-caldera-agent##\");filename=obas-implant-caldera-$(tr -dc A-Za-z0-9 </dev/urandom | head -c 5; echo);server=\"" + calderaInjectorConfig.getPublicUrl() + "\";curl -s -X GET " + openBASConfig.getBaseUrl() + "/api/implant/caldera/macos/x86_64 > $location/$filename;chmod +x $location/$filename;$location/$filename -server $server -group red &");
        executorCommands.put(Endpoint.PLATFORM_TYPE.MacOS.name() + "." + Endpoint.PLATFORM_ARCH.arm64, "x=\"#{location}\";location=$(echo \"$x\" | sed \"s#/openbas-caldera-agent##\");filename=obas-implant-caldera-$(tr -dc A-Za-z0-9 </dev/urandom | head -c 5; echo);server=\"" + calderaInjectorConfig.getPublicUrl() + "\";curl -s -X GET " + openBASConfig.getBaseUrl() + "/api/implant/caldera/macos/arm64 > $location/$filename;chmod +x $location/$filename;$location/$filename -server $server -group red &");
