...
public class OpenBASInjector {
  ...
  private String dlUri(OpenBASConfig openBASConfig, String platform, String arch) {
    return "\""
        ...
        + "/"
        + arch
        + "?executionId=#{execution}&agentId=#{agent}\"";
  }
  ...
  public OpenBASInjector(
    ...
    executorCommands.put(
        ...
        "$x=\"#{location}\";$location=$x.Replace(\"\\obas-agent-caldera.exe\", \"\");[Environment]::CurrentDirectory = $location;$filename=\"obas-implant-#{execution}-agent-#{agent}.exe\";$"
            + tokenVar
            + ";$"
            + serverVar
            + ";$"
    ...
    executorCommands.put(
        ...
        "x=\"#{location}\";location=$(echo \"$x\" | sed \"s#/openbas-caldera-agent##\");filename=obas-implant-#{execution}-agent-#{agent};"
            + serverVar
            + ";"
            + tokenVar
            + ";"
            ...
            + " > $location/$filename;chmod +x $location/$filename;$location/$filename --uri $server --token $token --unsecured-certificate $unsecured_certificate --with-proxy $with_proxy --agent-id #{agent} --execution-id #{execution} &");
    executorCommands.put(
