...
public class AtomicTestingUtils {
  ...
  public static List<InjectTargetWithResult> getTargetsWithResultsFromRaw(
    ...
    // Check if each team defined in an inject has an expectation. If not, create a result with default expectations
    if (rawTeamMap != null) {
    ...
    // Check if each asset defined in an inject has an expectation. If not, create a result with default expectations
    if (rawAssetMap != null) {
    ...
    // Check if each asset group defined in an inject has an expectation. If not, create a result with default expectations
          ...
          teamExpectations
              ...
              .collect(
                  Collectors.groupingBy(RawInjectExpectation::getTeam_id,
                      Collectors.collectingAndThen(
                          Collectors.toList(), AtomicTestingUtils::getExpectationResultByTypesFromRaw)
                  )
              )
              .entrySet().stream()
    ...
    if (!assetExpectations.isEmpty()) {
      // Assets are saves in assetsToRefine because we need check if the asset is linked to the inject and, at the same time, also linked to the asset group
      assetsToRefine.addAll(
          assetExpectations
              .stream()
              .collect(
      ...
      targets.addAll(assetGroupExpectations
          ...
          .map(entry -> {
            ...
            for (InjectTargetWithResult asset : assetsToRefine) {
              ...
              if (foundExpectationForAsset || foundExpectationForDynamicAssets) {
                children.add(asset);
                // We add the assetId to the removal list for assets that are directly linked to the inject
                assetsToRemove.add(asset);
              }
            }
    ...
    // Compare the assets directly linked to the inject {injectAssetIds} to retain only the results from these assets
    assetsToRefine.removeAll(
        assetsToRemove
            .stream()
            .filter(
  ...
  private static List<InjectTargetWithResult> calculateResultsforPlayersFromRaw(
    ...
    return expectationsByUser.entrySet().stream()
        .map(userEntry -> new InjectTargetWithResult(
            TargetType.PLAYER,
            userEntry.getKey(),
            StringUtils.getName(rawUserMap.get(userEntry.getValue().get(0).getUser_id()).getUser_firstname(), rawUserMap.get(userEntry.getValue().get(0).getUser_id()).getUser_lastname()),
  ...
  public static List<ExpectationResultsByType> getExpectationResultByTypesFromRaw(
      List<RawInjectExpectation> expectations) {
    List<Double> preventionScores = getScoresFromRaw(List.of(EXPECTATION_TYPE.PREVENTION), expectations);
    List<Double> detectionScores = getScoresFromRaw(List.of(EXPECTATION_TYPE.DETECTION), expectations);
    List<Double> humanScores = getScoresFromRaw(
        List.of(EXPECTATION_TYPE.ARTICLE, EXPECTATION_TYPE.CHALLENGE, EXPECTATION_TYPE.MANUAL), expectations);
  ...
  public static List<Double> getScoresFromRaw(List<EXPECTATION_TYPE> types,
      List<RawInjectExpectation> expectations) {
    return expectations
        .stream()
