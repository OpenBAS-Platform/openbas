...
public class ExerciseApi extends RestBehavior {
  ...
  public ExerciseDetails exercise(@PathVariable String exerciseId) {
    ...
    Map<String, List<RawEvaluation>> mapEvaluationsByObjective = evaluationRepository
            .rawByObjectiveIds(rawObjectives.stream()
  ...
  public Exercise changeExerciseStatus(
    ...
    if (isCloseState && ExerciseStatus.SCHEDULED.equals(status)) {
      ...
      List<LessonsAnswer> lessonsAnswers = lessonsCategoryRepository
          .findAll(LessonsCategorySpecification.fromExercise(exerciseId)).stream().flatMap(
