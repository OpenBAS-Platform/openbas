...
public class NotificationRuleApi {
    ...
    public NotificationRuleApi(NotificationRuleService notificationRuleService, NotificationRuleMapper notificationRuleMapper) {
        super();
    ...
    public NotificationRuleOutput findNotificationRule(
            @PathVariable @NotBlank @Schema(description = "ID of the notification rule") final String notificationRuleId) {
        return notificationRuleService.findById(notificationRuleId).map(notificationRuleMapper::toNotificationRuleOutput).orElse(null);
    }
    ...
    @ApiResponses(
            value = {@ApiResponse(responseCode = "200", description = "The list of all NotificationRules")})
    public List<NotificationRuleOutput> notificationRules() {
        return notificationRuleService.findAll().stream().map(notificationRuleMapper::toNotificationRuleOutput).toList();
    }
    ...
    public NotificationRuleOutput createNotificationRule(@Valid @RequestBody final CreateNotificationRuleInput input) {
        return notificationRuleMapper.toNotificationRuleOutput(
                notificationRuleService.createNotificationRule(notificationRuleMapper.toNotificationRule(input)));
    }
    ...
    public Page<NotificationRuleOutput> searchNotificationRules(
        ...
        return notificationRuleService.searchNotificationRule(searchPaginationInput).map(notificationRuleMapper::toNotificationRuleOutput);
    }
}
