...
public class ExpectationApi extends RestBehavior {
  ...
  public List<InjectExpectation> getInjectExpectationsAssetsNotFilledForSource(
    ...
    results = results.stream().filter(injectExpectation -> injectExpectation.getInject().getStatus().isPresent() && injectExpectation.getInject().getStatus().get().getTrackingEndDate() != null).toList();

    results.forEach(e -> e.setAsset((Endpoint) Hibernate.unproxy(e.getAsset())));
