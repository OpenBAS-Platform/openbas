...
public class ExpectationApi extends RestBehavior {
    ...
    @Autowired
    public void setExerciseExpectationService(final ExerciseExpectationService exerciseExpectationService) {
    ...
    public InjectExpectation updateInjectExpectation(@PathVariable @NotBlank final String expectationId, @Valid @RequestBody @NotNull InjectExpectationUpdateInput input) {
        ...
        List<InjectExpectation> expectationAssetGroups = inject.getExpectations().stream().filter(e -> e.getAssetGroup() != null).toList();
