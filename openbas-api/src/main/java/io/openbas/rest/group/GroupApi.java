...
public class GroupApi extends RestBehavior {
    ...
    public Page<Group> users(@RequestBody @Valid final SearchPaginationInput searchPaginationInput) {
        return buildPaginationJPA(
                (Specification<Group> specification, Pageable pageable) -> this.groupRepository.findAll(
    ...
    public Group updateGroupUsers(@PathVariable String groupId,
        ...
        exerciseRepository.saveAll(savedGroup.getGrants()
                .stream()
                .map(Grant::getExercise)
                .filter(Objects::nonNull)
                .collect(Collectors.toList()));
        // Publish scenarios impacted by this group change.
        scenarioRepository.saveAll(savedGroup.getGrants()
                .stream()
