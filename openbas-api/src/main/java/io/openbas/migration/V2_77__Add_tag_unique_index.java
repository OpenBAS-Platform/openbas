...
public class V2_77__Add_tag_unique_index extends BaseJavaMigration {
  ...
  public void migrate(Context context) throws Exception {
    ...
    try (Statement statement = connection.createStatement()) {
      ...
      recreatePrimaryConstraint(statement, "asset_groups_tags", "asset_groups_tags_pkey", "asset_group_id");
      recreatePrimaryConstraint(statement, "assets_tags", "assets_tags_pkey", "asset_id");
      recreatePrimaryConstraint(statement, "challenges_tags", "challenges_tags_pkey", "challenge_id");
      ...
      recreatePrimaryConstraint(statement, "organizations_tags", "organizations_tags_pkey", "organization_id");
  ...
  private static void deduplicateInTable(Statement statement, String table, String constraintKey) throws SQLException {
    // Remove primary key
    statement.executeUpdate("""
        ALTER TABLE """ + " ".concat(table).concat(" ") + """
            DROP CONSTRAINT  """ + " ".concat(constraintKey).concat(" ") + """
        """);
    // Set uniq tag
    statement.executeUpdate("""
    ...
    statement.executeUpdate("""
        CREATE TABLE """ + " ".concat(table).concat("_temp") + """
            AS
          SELECT DISTINCT * FROM """ + " ".concat(table).concat(" ") + """
        """);
    statement.executeUpdate("""
        DROP TABLE """ + " ".concat(table).concat(" ") + """
        """);
    statement.executeUpdate("""
        ALTER TABLE """ + " ".concat(table).concat("_temp") + """
          RENAME
        TO """ + " ".concat(table).concat(" ") + """
        """);
  }

  private static void recreatePrimaryConstraint(Statement statement, String table, String constraintKey, String key)
      throws SQLException {
    // Add primary key
    statement.executeUpdate("""
        ALTER TABLE """ + " ".concat(table).concat(" ") + """
          ADD CONSTRAINT """ + " ".concat(constraintKey).concat(" ") + """
          PRIMARY KEY (
            """ + " ".concat(key).concat(" ") + """
            , tag_id
            );
