...
public class V2_45__Media_upgrade extends BaseJavaMigration {
    ...
    public void migrate(Context context) throws Exception {
        ...
        select.execute("ALTER TABLE exercises ADD constraint fk_exercise_logo_dark foreign key (exercise_logo_dark) references documents on delete cascade;");
        select.execute("ALTER TABLE exercises ADD constraint fk_exercise_logo_light foreign key (exercise_logo_light) references documents on delete cascade;");
        select.execute("ALTER TABLE medias ADD constraint fk_media_logo_dark foreign key (media_logo_dark) references documents on delete cascade;");
        select.execute("ALTER TABLE medias ADD constraint fk_media_logo_light foreign key (media_logo_light) references documents on delete cascade;");
    }
}
