...
public class NotificationRuleServiceTest {
    ...
    public void test_activateNotificationRules(){
        ...
        when(notificationRuleRepository.
                findNotificationRuleByResourceAndTrigger(rule.getResourceId(), rule.getTrigger())).thenReturn(List.of(rule));

        notificationRuleService.activateNotificationRules(rule.getResourceId(), rule.getTrigger(), data);
