...
public class WithMockUserFullPermissionsSecurityContextFactory
    implements WithSecurityContextFactory<WithMockUserFullPermissions>, ApplicationListener<ContextRefreshedEvent> {
  ...
  public void preDestroy() {
    this.userRepository.deleteById(
        this.userRepository.findByEmailIgnoreCase(MOCK_USER_FULL_PERMISSION_EMAIL).orElseThrow().getId());
    this.groupRepository.deleteById(this.groupRepository.findOne(GroupSpecification.fromName(MOCK_FULL_PERMISSION_GROUP))
        .orElseThrow()
        .getId());;
    this.roleRepository.deleteById(this.roleRepository.findOne(RoleSpecification.fromName(MOCK_ROLE_FULL_PERMISSION_NAME))
        .orElseThrow()
        .getId());
  }
  ...
  public void createPlannerMockUser() {
    Optional<User> userOpt = this.userRepository.findByEmailIgnoreCase(MOCK_USER_FULL_PERMISSION_EMAIL);
    if (userOpt.isPresent() && userOpt.get().isPlanner()) {
      return;
    }
    // Create role
    Optional<Role> roleOpt = this.roleRepository.findOne(RoleSpecification.fromName(MOCK_ROLE_FULL_PERMISSION_NAME));
    Role role;
    if (roleOpt.isEmpty()) {
