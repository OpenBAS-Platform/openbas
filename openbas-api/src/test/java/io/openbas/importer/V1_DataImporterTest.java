...
class V1_DataImporterTest extends IntegrationTest {
  ...
  void testScenario_with_attackpattern() throws IOException {
    ...
    String jsonContent =
            new String(
                    Files.readAllBytes(Paths.get("src/test/resources/importer-v1/import-scenario-with-attack-pattern.json")));
    this.importNode = mapper.readTree(jsonContent);
    this.importer.importData(this.importNode, Map.of(), null, null);
    ...
    //delete scenario and payload before reimporting to verify that the killchainphase is not recreated
