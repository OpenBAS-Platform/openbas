...
class ObjectiveController extends Controller
{
    ...
    public function getExercisesObjectivesAction(Request $request)
    {
        ...
        return $exercise->getExerciseObjectives();
    }
    ...
    /**
     * @ApiDoc(
     *    description="Replace an objective",
     *   input={"class"=ObjectiveType::class, "name"=""}
     * )
     *
     * @Rest\View(serializerGroups={"objective"})
    ...
    {
        return $this->updateObjective($request, true);
    }
     ...
     *
     * @Rest\View(serializerGroups={"objective"})
     * @Rest\Patch("/exercises/{exercise_id}/objectives/{objective_id}")
     */
    public function patchExercisesObjectiveAction(Request $request)
    {
        return $this->updateObjective($request, false);
    }

    private function updateObjective(Request $request, $clearMissing)
    {
        $em = $this->get('doctrine.orm.entity_manager');
        $exercise = $em->getRepository('APIBundle:Exercise')->find($request->get('exercise_id'));
        /* @var $exercise Exercise */
