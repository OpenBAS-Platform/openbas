...
@Slf4j
/**
 * Base class for one-shot tenant-scoped data migrations. Subclasses implement {@link
 * #doMigrate(Tenant)} which is executed exactly once per tenant (idempotency tracked via {@link
 * DataPackService}).
 *
 ...
 * #process(Tenant)} call, idempotency check included. A subclass's {@link #doMigrate(Tenant)} must
 * NOT open its own transaction/scope — it runs inside the caller's transaction and inherits its
 * scope automatically.
 */
public abstract class RuntimeMigration implements Processable {
  ...
  protected abstract boolean doMigrate(Tenant tenant);

  /**
   * Enables the v1 Hibernate {@code tenantFilter} for the tenant being processed. Call this
   * explicitly at the top of {@link #doMigrate(Tenant)} when the migration reads v1-scoped entities
   * through JPQL / derived queries that are not explicitly tenant-parameterized (e.g. {@code
