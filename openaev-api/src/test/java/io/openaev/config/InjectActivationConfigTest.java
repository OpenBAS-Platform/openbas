...
/**
 * Guards that the production configuration keeps {@code injects} activated in v2. The table is
 ...
 */
@DisplayName("Production config keeps injects activated (v2)")
class InjectActivationConfigTest {

  @Test
  @DisplayName("openaev.tenant.active-tables in application.properties contains injects")
  void prodConfigActivatesInjects() throws Exception {
    ...
    // Exact token comparison, not substring: a value like "injects_archive" must not satisfy this
    // guard once the real table has been deactivated.
    List<String> tables = Arrays.stream(active.split(",")).map(String::trim).toList();
    assertTrue(
        tables.contains("injects"),
        "injects must stay in openaev.tenant.active-tables: its v1 @Filter was removed, so"
