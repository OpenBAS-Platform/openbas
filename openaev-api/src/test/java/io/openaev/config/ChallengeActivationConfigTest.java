...
/**
 * Guards that the production configuration keeps {@code challenges} activated in v2. The table is
 ...
 */
@DisplayName("Production config keeps challenges activated (v2)")
class ChallengeActivationConfigTest {

  @Test
  @DisplayName("openaev.tenant.active-tables in application.properties contains challenges")
  void prodConfigActivatesChallenges() throws Exception {
    ...
    assertTrue(
        active.contains("challenges"),
        "challenges must stay in openaev.tenant.active-tables: its v1 @Filter was removed, so"
