...
public class Inject implements Base, Injection {
  ...
  public Instant computeInjectDate(Instant source, int speed) {
    return InjectModelHelper.computeInjectDate(
        source, speed, getDependsOn(), getDependsDuration(), getExercise());
  }
  ...
  public Optional<Instant> getDate() {
    ...
    return InjectModelHelper.getDate(
        getExercise(), getScenario(), getDependsOn(), getDependsDuration());
  }
