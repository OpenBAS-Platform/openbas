...
public class InjectModelHelper {
  ...
  public static Instant computeInjectDate(
      Instant source, int speed, Inject dependsOn, Long dependsDuration, Exercise exercise) {
    ...
    Instant dependingStart =
        dependsOnInject.map(inject -> inject.computeInjectDate(source, speed)).orElse(source);
  ...
  public static Optional<Instant> getDate(
      Exercise exercise, Scenario scenario, Inject dependsOn, Long dependsDuration) {
    ...
    if (exercise != null) {
      ...
      return exercise
          ...
          .map(
              source ->
                  computeInjectDate(source, SPEED_STANDARD, dependsOn, dependsDuration, exercise));
    }
