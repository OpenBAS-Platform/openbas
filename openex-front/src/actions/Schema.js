...
const _buildUser = (state, usr) => {
  ...
  return {
    ...
    tags: usr.user_tags.map((tagId) => state.referential.entities.tags[tagId])
      .filter((t) => t !== undefined),
    organization: state.referential.entities.organizations[usr.user_organization],
    // eslint-disable-next-line max-len
    tokens: R.filter((n) => n.token_user === usr.user_id, R.values(state.referential.entities.tokens)),
  };
};
...
const _buildExercise = (state, id, ex) => {
  ...
  return {
    ...
    users: getAudiences().map((a) => a.users).flat(),
  };
};
...
export const storeBrowser = (state) => ({
  ...
  organizations: R.values(state.referential.entities.organizations)
    .map((org) => _buildOrganization(state, org)),
  documents: R.values(state.referential.entities.documents)
    .map((doc) => _buildDocument(state, doc.document_id, doc)),
  exercises: R.values(state.referential.entities.exercises)
    .map((ex) => _buildExercise(state, ex.exercise_id, ex)),
  settings: R.mergeAll(Object.entries(state.referential.entities.parameters ?? {})
    .map(([k, v]) => ({ [k]: v.setting_value }))),
  me: _buildUser(state.referential.entities.users[R.path(['logged', 'user'], state.app)]),
