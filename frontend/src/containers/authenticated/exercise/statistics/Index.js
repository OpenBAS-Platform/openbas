...
class Index extends Component {
  constructor(props) {
    ...
    keyArrayStatistics.forEach(function (key) {
      oStatistics[key] = ''
    })
  ...
  handleChange(event, index, value) {
    ...
    this.props.getStatisticsForExercise(this.props.exerciseId, data).then(statistics => {
  ...
  handleGetStatistics(data) {
    let oStatistics = this.state
    keyArrayStatistics.forEach(function (key) {
      oStatistics[key] = data[key]
    })

    //Construct data for graph bar inject per player
    let oInjectPerPlayer = this.state.injectPerPlayer

    let labelArrayInjectPerPlayer = []
    if (oInjectPerPlayer['labels']) {
      oInjectPerPlayer['labels'].forEach(function (label) {
        labelArrayInjectPerPlayer.push(label)
      })
    }

    let dataArrayInjectPerPlayer = []
    if (oInjectPerPlayer['datasets'] && oInjectPerPlayer['datasets']['data']) {
      oInjectPerPlayer['datasets']['data'].forEach(function (data) {
    ...
    let oInjectPerIncident = this.state.injectPerIncident

    let labelArrayInjectPerIncident = []
    if (oInjectPerIncident['labels']) {
      oInjectPerIncident['labels'].forEach(function (label) {
        labelArrayInjectPerIncident.push(label)
      })
    }

    let dataArrayInjectPerIncident = []
    if (oInjectPerIncident['datasets'] && oInjectPerIncident['datasets']['data']) {
      oInjectPerIncident['datasets']['data'].forEach(function (data) {
    ...
    let oInjectPerInterval = this.state.injectPerInterval

    let labelArrayInjectPerInterval = []
    if (oInjectPerInterval['labels']) {
      oInjectPerInterval['labels'].forEach(function (label) {
        labelArrayInjectPerInterval.push(label)
      })
    }

    let dataArrayInjectPerInterval = []
    if (oInjectPerInterval['datasets'] && oInjectPerInterval['datasets']['data']) {
      oInjectPerInterval['datasets']['data'].forEach(function (data) {
  ...
  render() {
    return (
      <div style={styles.container}>
        <div style={styles.title}><h2><T>Statistics</T></h2></div>
        <div className="clearfix"></div>
        <div style={styles.lineStatistic}>
          <div style={styles.columnStatistic}>
            <p>
              Nombre total d'impressions pour cet exercice&nbsp;: {this.state.allInjectsCount}
              &nbsp;
              <span
                title="nombre d'impressions = somme (inject i * nombre de destinataires i) pour i de 1 à n."
                role="img"
                aria-label="info">ℹ️</span>
            </p>
            <p>
              Nombre moyen d'injects par joueur&nbsp;: {this.state.avgInjectPerPlayerCount}
            </p>
            <p>
              Nombre total de joueurs&nbsp;: {this.state.allPlayersCount}
            </p>
          </div>
          <div style={styles.columnStatistic}>
            <p>
              Nombre d'organisations&nbsp;: {this.state.organizationsCount}
            </p>
            <p>
              Fréquence des impressions&nbsp;: {this.state.frequencyOfInjectsCount} impression(s)/heure
            </p>
          </div>
        </div>
