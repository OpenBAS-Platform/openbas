...
i18nRegister({
  fr: {
    ...
    'Execute Tests after update':
      "Exécution de tests après une mise à jour de l'application.",
    ...
    'Reimport of the test exercise... OK':
      "Réimport de l'exercice de test... OK",
    ...
    'An error occurred while creating the test exercise':
      "Une erreur est survenue lors de la création de l'exercice de test",
    'An error occurred while exporting the exercise':
      "Une erreur est survenue lors de l'export de l'exercice",
    'An error occurred while deleting test user accounts':
      'Une erreur est survenue lors de la suppression des comptes utilisateur de test',
    'An error occurred while importing the test exercise':
      "Une erreur est survenue lors de l'importation de l'exercice de test",
    'Creating a test exercise': "Création d'un exercice de test",
    'Export the test exercise': 'Export de cet exercice',
    'Deleting the test exercise':
      "Suppression de l'exercice en base de données",
...
class Index extends Component {
  ...
  updateTestResult(step, result) {
    const { resultTest } = this.state;
    resultTest[step] = result;
    this.setState({ resultTest });
  }
  ...
  submitTests(data) {
    ...
    const now = Date.now();
    const exportFileName = `../var/files/test_${now}.xls`;
    ...
    this.props.createExerciseTest(data).then((resultCreateExercise) => {
      if (resultCreateExercise.result.success === true) {
        ...
        const paramExportExercise = this.state.typesToExport;
        ...
        this.props
          .exportExercise(
            resultCreateExercise.result.exercise_id,
            paramExportExercise,
          )
          .then((resultExportExercise) => {
            if (resultExportExercise.result.success === true) {
              ...
              this.props
                .deleteExercise(resultCreateExercise.result.exercise_id)
                .then((resultDeleteExercise) => {
                  ...
                  this.props.deleteUsersTest().then((resultDeleteUsers) => {
                    if (resultDeleteUsers.result.success === true) {
                      ...
                      const paramImportExercise = this.state.typesToExport;
                      paramImportExercise.import_path = exportFileName;
                      this.updateResultTestDesc(
                        'Reimport of the test exercise...',
                      );
                      this.updateTestResult('step5', 'doing');
                      this.props
                        .importExerciseFromPath(paramImportExercise)
                        .then((resultImportExercise) => {
                          if (resultImportExercise.result.success === true) {
                            ...
                            // try to run dryrun
                            const paramAddDryrun = { dryrun_speed: 72 };
                            this.updateResultTestDesc(
                              'Execution of the Dryrun...',
                            );
                            this.updateTestResult('step6', 'doing');
                            this.props
                              .addDryrun(
                                resultImportExercise.result.exercise_id,
                                paramAddDryrun,
                              )
                              .then((resultAddDryrun) => {
                                this.updateResultTestDesc(
                                  'Execution of the Dryrun... OK',
                                );
                                ...
                                this.props
                                  ...
                                  .then((resultDeleteExercise) => {
                                    this.props
                                      .deleteUsersTest()
                                      .then((resultDeleteUsers) => {
                                        this.updateResultTestDesc(
                                          'Deleting test data... OK',
                                        );
                                        ...
                                        if (
                                          resultDeleteUsers.result.success
                                          === true
                                        ) {
                                          this.updateResultTestDesc(
                                            'Deleting test accounts... OK',
                                          );
  ...
  updateResultTestDesc(description) {
    const { resultTestDesc } = this.state;
  ...
  renderUnitTestStatus(step) {
    return this.state.resultTest[step] === false ? (
    ...
    return (
      <div>
        <div style={styles.title}>
          <T>Execute Tests after update</T>
        </div>
        ...
        <Paper type={Constants.PAPER_TYPE_SETTINGS} zDepth={2}>
          <div style={styles.PaperContent}>
            ...
            <ul>
              <li style={styles.li}>
                {this.renderUnitTestStatus('step1')}&nbsp;
                <T>Creating a test exercise</T>
              </li>
              <li style={styles.li}>
                {this.renderUnitTestStatus('step2')}&nbsp;
                <T>Export the test exercise</T>
              </li>
              <li style={styles.li}>
                {this.renderUnitTestStatus('step3')}&nbsp;
                <T>Deleting the test exercise</T>
              </li>
              <li style={styles.li}>
                {this.renderUnitTestStatus('step4')}&nbsp;
                <T>Import of the test exercise</T>
              </li>
              <li style={styles.li}>
                {this.renderUnitTestStatus('step5')}&nbsp;
                <T>Execute a dryrun</T>
              </li>
              <li style={styles.li}>
                {this.renderUnitTestStatus('step6')}&nbsp;
                <T>Deleting the test exercise</T>
              </li>
              <li style={styles.li}>
                {this.renderUnitTestStatus('step7')}&nbsp;
                <T>Deleting test accounts</T>
              </li>
            </ul>
            <Button
              label="Run the test"
              onClick={this.handleSubmitTests.bind(this)}
            />
          </div>
          <div style={styles.PaperContent}>
            {this.state.resultTestDesc.map((key, item) => (
                <div>
                  <T>{key}</T>
                </div>
            ))}
