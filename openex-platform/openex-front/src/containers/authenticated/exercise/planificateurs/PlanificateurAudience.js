...
const PlanificateurAudience = ({
   ...
   submitFormPlanificateur
    ...
    const planificateurActions = [
      <FlatButton key="cancel" label="Cancel" primary={true} onClick={handleClosePlanificateur}/>,
      <FlatButton key="submit" label="Submit" primary={true} onClick={submitFormPlanificateur}/>,                
    ]
    
    const handleCheck = (planificateurUserId, audienceId) => (event, isChecked) => {
        handleCheckPlanificateur(planificateurUserId, audienceId, isChecked)
    }
    
    return (
        <Dialog title="Please Choose Planners" modal={false} 
                ...
                <form onSubmit={submitFormPlanificateur}>
                    <Table selectable={false} style={{marginTop: '5px'}}>
                        <TableHeader adjustForCheckbox={false} displaySelectAll={false}>
                            <TableRow>
                                <TableHeaderColumn><T>FirstName</T></TableHeaderColumn>
                                <TableHeaderColumn><T>LastName</T></TableHeaderColumn>
                                <TableHeaderColumn><T>Email</T></TableHeaderColumn>
                        ...
                        {R.values(planificateursAudience).map(planificateur => {
                                return (
                                    <TableRow key={planificateur.user_id}>
                                       <TableRowColumn>{planificateur.user_firstname}</TableRowColumn>
                                       <TableRowColumn>{planificateur.user_lastname}</TableRowColumn>
                                       <TableRowColumn>{planificateur.user_email}</TableRowColumn>
                                       <TableRowColumn>
                                           <Checkbox defaultChecked={planificateur.is_planificateur_audience} onCheck={handleCheck(planificateur.user_id, audienceId)} />
                                       </TableRowColumn>
                                    </TableRow>
                                )
                            }
